...
const RemoveBackground = () => {
  ...
  const onSubmitHandler = async (e) => {
    ...
    try {
      ...
      const { data } = await axios.post(
        `${VITE_BASE_URL}/api/ai/remove-image-background`,
        formData,
        {
          headers: {
  ...
  return (
    <div
      className="h-full overflow-y-scroll p-6 flex items-start flex-wrap gap-4 text-slate-700 w-10000  flex-row"
      style={{ maxWidth: 1200 }}
    >
      {/*left col*/}
      <form
        onSubmit={onSubmitHandler}
        className="w-full max-w-lg p-4 bg-white rounded-lg border border-gray-200 flex-row
        "
      >
        ...
        <input
          ...
          required
        />
        <p className="text-xs text-gray-500 font-light items-center gap-2">
